...
app.get('/scope', (req, res, next) => {
  (async () => {
    if (sessionToken) {
      ...
      xmlReq.setRequestHeader('Authorization', `Bearer ${sessionToken.access_token}`);
...
app.get('/cb', (req, res, next) => {
  (async () => {
    ...
    const tokenSet = await client.oauthCallback(`http://${HOST}:${PORT}/cb`, params, { code_verifier: codeVerifier, state }, { exchangeBody: { client_id: process.env.CLIENT_ID } });
